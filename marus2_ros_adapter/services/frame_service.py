...
try:
    ...
    try:
        ...
        from ..marus2_proto import tf_pb2
        from ..marus2_proto import std_pb2
    except ImportError:
        from marus2_ros_adapter.marus2_proto import tf_pb2_grpc
        from marus2_ros_adapter.marus2_proto import tf_pb2
...
class FrameService(tf_pb2_grpc.TfServicer):
    ...
    def __init__(self):
        ...
        rh.Subscription(TFMessage, "/tf", self.tf_callback, qos_profile=10)
        rh.Subscription(TFMessage, "/tf_static", self.static_tf_callback, qos_profile=10)
    ...
    def _find_tf_in_list(self, tf, l):
        return next(
            (
                ...
                and v.header.frame_id == tf.header.frame_id
            ),
            (-1, None),
        )
    ...
    def _standardize_tf(self, tf):
        """
        Some tfs have '/' character at the beginning of the name. It is removed.
        """
    ...
    def static_tf_callback(self, message):
        with self._static_tfs_lock:
            for new_tf in message.transforms:
                self._standardize_tf(new_tf)
                idx, existing_tf = self._find_tf_in_list(new_tf, self.static_tfs)
                if existing_tf is not None:
                    del self.static_tfs[idx]
    ...
    def GetAllFrames(self, request, context):
        return self._merge_frames_as_frame_list()
